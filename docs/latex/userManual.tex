...
A large quantity of useful technical data is available only as visual representations in image files (e.g. plots). These visualizations help in understanding the relationship between the variables involved, but recovering the exact numerical values of the data is usually very difficult or impossible. To aid this time consuming task of data recovery, many digitization tools have been developed over the years but this task remains daunting and prone to errors. Some common complains with the existing tools are as follows:
